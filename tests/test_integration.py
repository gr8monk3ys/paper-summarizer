...
class TestIntegration:
    ...
    def test_summarize_from_url(self, client, auth_headers):
        ...
        with (
            patch(
                "paper_summarizer.core.summarizer.PaperSummarizer.summarize_from_url"
            ) as mock_summarize,
    ...
    def test_summarize_from_text(self, client, auth_headers):
        ...
        with patch(
            "paper_summarizer.core.summarizer.PaperSummarizer.summarize"
        ) as mock_summarize:
            mock_summarize.return_value = TEST_DATA["sample_summary"]
    ...
    def test_batch_processing_flow(self, client, test_files, auth_headers):
        ...
        with patch(
            "paper_summarizer.core.summarizer.PaperSummarizer.summarize_from_file"
        ) as mock_summarize:
            mock_summarize.return_value = TEST_DATA["sample_summary"]
            ...
            for file_path in test_files:
                files.append(
                    ("files[]", (file_path.name, file_path.read_bytes(), "text/plain"))
                )

            response = client.post(
                "/batch", data=data, files=files, headers=auth_headers
            )
            ...
            assert all(
                s["summary"] == TEST_DATA["sample_summary"] for s in result["summaries"]
            )
    ...
    def test_error_handling(self, client, auth_headers):
        ...
        response = client.post(
            "/summarize", data=data, files=files, headers=auth_headers
        )
