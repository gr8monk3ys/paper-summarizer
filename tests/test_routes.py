...
def mock_summarizer():
    ...
    with (
        patch(
            "paper_summarizer.web.routes.summaries.PaperSummarizer"
        ) as mock_summaries,
...
class TestRoutes:
    ...
    def test_invalid_source_type(self, client, auth_headers):
        ...
        data = {"source_type": "invalid", "text": TEST_DATA["sample_text"]}
        response = client.post("/summarize", data=data, headers=auth_headers)
    ...
    @pytest.mark.parametrize("route", ["/library", "/batch", "/analytics", "/settings"])
