...
class TestPaperSummarizer:
    ...
    def summarizer(self):
        ...
        return PaperSummarizer(
            model_type=ModelType.T5_SMALL, provider=ModelProvider.LOCAL
        )
    ...
    def test_summarize_with_local_model(self, summarizer):
        ...
        with patch.object(
            summarizer,
            "_summarize_local",
            return_value=TEST_DATA["sample_summary"].strip(),
        ):
    ...
    def test_summarize_with_together_ai(self):
        ...
        with patch("together.Complete.create") as mock_create:
            mock_create.return_value = {
                "output": {"choices": [{"text": TEST_DATA["sample_summary"].strip()}]}
            }
    ...
    def test_citation_handling(self, summarizer):
        ...
        assert " ".join(processed_text.split()) == " ".join(
            text_without_citations.split()
        )

        # Test summarization with citations
        with patch.object(
            summarizer, "_summarize_local", return_value=TEST_DATA["sample_summary"]
        ):
            # Keep citations
